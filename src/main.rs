...
fn main() -> Result<(), io::Error> {
    ...
    let terminal = MouseTerminal::from(stdout);
    let backend = TermionBackend::new(terminal);
    ...
    let mut current_history: Vec<String> = std::fs::read_to_string(&shell.config.history_path)
        .unwrap()
        .split("\n")
        .map(|elem| {
    ...
    loop {
        ...
        terminal
            .draw(|f| {
                ...
                let active_pane = Span::styled(
                    format!("Interacting With: {}", shell.active_pane),
                    Style::default()
                        .fg(Color::Yellow)
                        .add_modifier(Modifier::BOLD),
                );
                ...
                let status_line = Paragraph::new(shell.input.as_ref())
                    .block(Block::default().borders(Borders::ALL).title(status_info));

                f.render_widget(status_line, shell.chunks[1]);
            })
            .unwrap();

        if let input::TerminalEvent::Input(input) = events.next().unwrap() {
                        ...
                        match key {
                                ...
                                match args[0] {
                                    ...
                                    "clear" => {
                                        ...
                                        let index = match args[1].parse::<usize>() {
                                            ...
                                            Err(_) => {
                                                shell.error =
                                                    "Incorrect arguments were provided".to_owned();
                                                shell.input.clear();
                                                continue;
                                            }
                                        };
                                        ...
                                        let value =
                                            if index <= 1 { 0 } else { (index - 1) as usize };
                                    ...
                                    "exit" => {
                                        shell
                                            .history
                                            .write_all(current_history.join("").as_bytes())
                                            .unwrap();
                                        break;
                                    }
                            ...
                            Key::Up => {
                                ...
                                if current_history.len() > history_index {
                                    ...
                                    shell.input = String::from(command).replace("\n", "");
                                    history_index += 1;
                                    shell
                                        .cursor
                                        .move_cursor(command.len() as u16, shell.cursor.get_y());
                                }
                            }
                                ...
                                match history_index.cmp(&0) {
                                    Ordering::Greater => {
                                        if history_index - 1 > 0 {
                                            let command =
                                                history_cloned[history_index - 1].trim_end();
                                            shell.input = String::from(command).replace("\n", "");
                                            history_index -= 1;
                                            shell.cursor.move_cursor(
                                                command.len() as u16,
                                                shell.cursor.get_y(),
                                            );
                                        } else {
